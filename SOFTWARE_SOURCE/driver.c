...
void i2c_master_init() {
	...
	Xil_Out32(PRESCALE_REG, 0x06);				//configure prescaler reg
}
...
void i2c_write(u32 reg_addr, u32 data) {
	Xil_Out32(ADDR_REG, ADDR_SLAVE_WRITE);
	Xil_Out32(TRANSMIT_REG, reg_addr);
	Xil_Out32(TRANSMIT_REG, data);
	Xil_Out32(CMD_REG, 0x20);
}
void i2c_slave_config() {
	i2c_write(SLAVE_CONFIG_REG, 0x08);
	Xil_In32(STATUS_REG);
}
